...
-- ==========================================
-- UPI Settle-Up (Payment Handles)
-- ==========================================
-- A UPI ID lives here rather than on `profiles`, which is governed by
-- "Public Profiles are viewable by everyone" (USING (true)) — every
-- authenticated user can read every row. That is fine for a display name, and
-- flows like invite previews depend on it, but it would publish every user's
-- payee address to every user. A separate table gets a strict policy of its
-- own without weakening one that existing features rely on.
...
-- "Does the caller share at least one group with this user?" — the
-- payment-handle analogue of is_group_member(). SECURITY DEFINER so the policy
-- can read group_members without recursing through its own RLS.
